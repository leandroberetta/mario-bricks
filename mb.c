...
void destroy(ALLEGRO_DISPLAY *display,
		...
		ALLEGRO_TIMER *timer, ALLEGRO_SAMPLE *sample,
		ALLEGRO_SAMPLE *sample_win, ALLEGRO_BITMAP *image,
		ALLEGRO_BITMAP *image_bg, ALLEGRO_FONT *font_title,
...
int main(int argc, char **argv) {
	...
	if (!al_init())
		al_show_native_message_box(NULL, NULL, "Error",
				"No se ha podido inicializar Allegro", NULL,
	...
	while (1) {
		...
		case ALLEGRO_EVENT_DISPLAY_CLOSE:
			destroy(display, event_queue, timer, sample, sample_win, image,
					image_bg, font_title, font_info);
		...
		case ALLEGRO_EVENT_KEY_DOWN:
			...
			case ALLEGRO_KEY_ENTER:
				...
				if (brick < BRICKS
						&& ((brick == 0)
								|| (brick > 0 && vec_brick[brick - 1] == x))) {
					vec_brick[brick] = x;
					brick++;
					y = y - 50;

					al_play_sample(sample, 1.0, 0.0, 1.0, ALLEGRO_PLAYMODE_ONCE,
							NULL);
				}
				...
				if (brick == 10) {
					al_play_sample(sample_win, 1.0, 0.0, 1.0,
		...
		case ALLEGRO_EVENT_TIMER:
			...
			al_draw_bitmap(image_bg, 0, 0, 0);
			al_draw_text(font_title, al_map_rgb(0, 0, 0), 800, 10,
					ALLEGRO_ALIGN_RIGHT, APP_TITLE);
			sprintf(progress, "tries: %d - progress: %d/%d ", tries, brick,
					BRICKS);
			al_draw_text(font_info, al_map_rgb(0, 0, 0), 800, 50,
					ALLEGRO_ALIGN_RIGHT, progress);
	...
	destroy(display, event_queue, timer, sample, sample_win, image, image_bg,
